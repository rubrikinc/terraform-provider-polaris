...
description: |-
  ...
  RSC Managed Host - When an RSC managed host configuration is created, RSC
  ...
  RSC or by the customer.Customer Managed Host - When a customer managed host configuration is
  created, RSC will not deploy any resources. Instead it will use the AWS EKS
  cluster attached by the customer, using the
  polaris_aws_exocompute_cluster_attachment resource, for all operations.Application - An application configuration is created by mapping the
  ...
  Using the account_id, region, vpc_id and subnets fields creates an
  ...
  configuration where the customer manage the security groups.Using the account_id and region fields creates a customer managed host
  ...
  configuration.Using the account_id and host_cloud_account_id fields creates an
...
import {
  to = polaris_aws_exocompute.application
  id = "app-58e2a8bb-078d-4f67-8b66-5515fd701c8e"
}
```
