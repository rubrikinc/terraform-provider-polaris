...
description: |-
  ...
  RSC Managed Host - When a host configuration is created, RSC will
  ...
  account.Customer Managed Host - When a customer managed host configuration is
  created, RSC will not deploy any resources. Instead it will use the Azure
  AKS cluster attached by the customer, using the
  polaris_azure_exocompute_cluster_attachment resource, for all operations.Application - An application configuration is created by mapping the
  ...
  Using the cloud_account_id, region, subnet and
  pod_overlay_network_cidr fields creates an RSC managed host configuration.Using the cloud_account_id and region fields creates a customer managed
  ...
  configuration.Using the cloud_account_id and host_cloud_account_id fields creates an
...
import {
  to = polaris_azure_exocompute.application
  id = "app-a9caddfd-25bd-4327-85f6-fa698ed898b6"
}
```
