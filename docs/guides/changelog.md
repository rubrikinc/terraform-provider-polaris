...
* No longer require `subscription_id` when `object_type` is `AzureNativeResourceGroup` in the `polaris_object` data
  ...
  [[docs](../data-sources/object.md)]
* Add support for the `AZURE_POSTGRES_FLEXIBLE_SERVER_PROTECTION` feature in the `polaris_azure_permissions` data
...
* Add support for the `postgres_flexible_server_protection` feature in the `polaris_azure_subscription` resource,
...
* Add support for the `AzurePostgresFlexibleServer` object type in the `polaris_object` data source, resolving an
...
* Add support for the `CLOUD_SQL_PROTECTION` feature in the `polaris_gcp_project` resource, which enables backup and
...
* Add support for the `GitHubOrganization` and `GitHubRepository` object types in the `polaris_object` data source,
  ...
  [[docs](../data-sources/object.md)]
* Add support for the `GITHUB_OBJECT_TYPE` object type in the `polaris_sla_domain` resource, allowing GitHub objects
...
* Refresh the documentation templates for the `polaris_aws_cloud_cluster`, `polaris_azure_cloud_cluster`,
  `polaris_gcp_cloud_cluster`, `polaris_aws_exocompute`, `polaris_cdm_bootstrap`,
  `polaris_cdm_bootstrap_cces_aws`, `polaris_aws_cnp_account`, `polaris_aws_cnp_account_trust_policy` and
  `polaris_sla_domain` resources, and the `polaris_aws_cnp_permissions`,
  `polaris_azure_archival_location` and `polaris_sla_domain` data sources, so that the published
...
* New resource added for `polaris_aws_account_managed` which onboards an RSC-managed AWS account (Rubrik-hosted
...
* New resource added for `polaris_aws_account_managed_stack` which completes onboarding of an RSC-managed AWS
